...
- 管理员可在作业提交表中为已提交学生保存分数。管理员可单独导出成绩 Excel，其中包含所有学生的学号、姓名、提交状态、最后提交时间和分数；未评分或未提交时分数为 `0`。作业 ZIP 不包含成绩表。
- 作业管理页可一键导出全部作业和成绩。下载 ZIP 的最外层为 `全部作业和成绩/`，每个作业各有以作业标题命名的目录；根目录的 `成绩汇总.xlsx` 按学生汇总每个作业的分数与总分，未评分或未提交时为 `0`。
