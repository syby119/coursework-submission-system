...
- 页面支持中文与 English 切换；语言偏好保存在浏览器 cookie 中。界面、操作反馈、上传错误、日期格式和新导出的 Excel/文件名会随之切换，作业标题、说明、学生姓名和原始文件名等用户数据保持原文。
...
- 时间以 PostgreSQL UTC `timestamptz` 存储，界面以 `Asia/Shanghai` 显示，并按当前中文或英文界面格式化。
